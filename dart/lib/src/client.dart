...
class CountrylanguagesValidationException implements Exception {
  final List<String> errors;

  CountrylanguagesValidationException(this.errors);

  @override
  String toString() => 'CountrylanguagesValidationException: ${errors.join("; ")}';
}
...
/// Country Languages API client.
///
/// For more information, visit: https://apiverve.com/marketplace/countrylanguages?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [name] (required) - The Country 2 letter ISO code for which you want to get languages for data (e.g., ZW)
class CountrylanguagesClient {
  final String apiKey;
  ...
  CountrylanguagesClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/countrylanguages',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CountrylanguagesValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CountrylanguagesValidationException(errors);
    }
  }

  /// Execute a request to the Country Languages API.
  ///
  /// Parameters are validated before sending the request.
  Future<CountrylanguagesResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CountrylanguagesException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CountrylanguagesResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CountrylanguagesException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CountrylanguagesException('Resource not found');
      } else {
        throw CountrylanguagesException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CountrylanguagesException) rethrow;
      throw CountrylanguagesException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Country Languages API client.
class CountrylanguagesException implements Exception {
  final String message;

  CountrylanguagesException(this.message);

  @override
  String toString() => 'CountrylanguagesException: $message';
}
